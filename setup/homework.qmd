...
When an assignment is released, I will post a link to the assignment repository on Ed Discussion. Click the **Fork** button on GitHub to create a copy of the repository under your own account.

Because forking creates a new repository under your own account, **you should not use the original repository**, as you will be unable to push any changes back to GitHub. In other words, do not clone a repository with the name `bee-envsys-cornell/hwx.git` or `bee-envsys-cornell/labx.git`, but look for your fork, named like `<your-username>/hwx.git`. If you accidentally clone the original repository, do not worry: you can re-clone your fork and move any files you edited to the new location.
...
Eventually, you will need to convert your solution to a PDF. This can be a little tricky with Jupyter Notebooks. Before you do anything, **make sure you run all of your notebook cells *in order***. An overview of the choices you have to do this conversion are available [in this tutorial](../tutorials/notebook-to-pdf.qmd).

If you choose a different approach to writing your solution, such as Word or LaTeX, just make sure you compile/save as a PDF. If you are using Word, make sure to check the PDF to ensure that all of your figures and equations are rendered correctly.
