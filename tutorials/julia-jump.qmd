---
title: "Linear Optimization in Julia"
execute:
    error: true
    warning: true
format:
...
When defining variables, we were able to declare constraints on their values by specifying upper and lower bounds. However, we also have other constraints, which involve multiple decision variables. These are specified using the `@constraint` macro. We also can pass names for each constraint^[Constraint names aren't required, but are useful if you want to modify constraints or [get dual variables/shadow prices](http://localhost:4200/tutorials/julia-jump.html#dual-solutions).]. We will use `time` for the time constraint and `materials` for the materials constraint. These names must be unique.
