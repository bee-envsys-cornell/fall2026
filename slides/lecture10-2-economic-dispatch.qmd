...
Note that relaxing the demand constraint (increasing demand by one MWh) **worsens** the objective (increasing cost).

The convention in `JuMP` is that the shadow price $$\lambda_\text{demand}$$ is therefore negative, but this is not universal.
...
:::
::: {.column width=50%}
![CAISO Duck Curve](https://www.eia.gov/todayinenergy/images/2023.06.21/main.svg)
...
**Monday**: Capacity expansion --- deciding what to *build*, not just how to run what exists.

**Wednesday**: Mixed integer programming.

## Assessments

**Homework 7**: Due tomorrow at 9pm.

**Mini-Project 2**: Assigned today, due Thursday, November 12.
