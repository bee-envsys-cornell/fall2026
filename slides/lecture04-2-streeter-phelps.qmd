...
1. **Simplifies solving**: each closed form makes its own assumptions --- this one divides by $k_a - k_c$ and $k_a - k_n$, so equal rates need a separate formula ([appendix](#sp-solution)). Change the kinetics and you re-derive from scratch. Forward Euler does not care.
2. **Conditions change as a parcel travels** --- temperature driving $k_a$ through the day, changing $U$, or dispersion mixing neighbouring parcels. Then no single closed form covers the trip.
