...
1. Numerical solution defined more generally (analytic solution undefined when $k_a = k_c$ or $k_a = k_n$).
...
Plant load is not constant.

Often peaks with morning and evening flows. 

Suppose CBOD swings $\pm 40\%$ over 24 hours, everything else fixed.
...
xd = 0:0.02:L
departure_time = [0.0, 0.25, 0.75]                      # days: mean, peak, trough
...
for i in 1:length(departure_time)
    DO_parcel = [do_along_parcel(departure_time[i], x, sp_params) for x in xd]
    plot!(p_parcel, xd, DO_parcel, color=parcel_color[i], linewidth=2.5,
        label="left at $(Int(round(departure_time[i] * 24))) h")
end
...
- The closed-form Streeter-Phelps solution is undefined at $k_a = k_c$ or $k_a = k_n$, and assumes every rate is constant along the river --- reasons to possibly discretize.
