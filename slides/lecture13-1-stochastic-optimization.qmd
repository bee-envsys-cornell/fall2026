...
- Example of a network model: need to represent flows between sources and sinks, constrained by existence of network connections.
...
- **Linearity** 
- **Divisibility** (when not present: mixed integer LP)
- **Certainty**
...
But most systems problems aren't ***truly*** deterministic.
...
2. Stress-test solution with Monte Carlo or include additional safety margins.
:::
...
## Act-Learn-Act (Two-Stage)

1. Make a decision $x$ under uncertainty;
2. Observe uncertainty $\xi \in \Xi$;
3. Make a **recourse** decision $q(x; \xi)$ which compensates for suboptimality of $x$ under realization $\xi$.

## Example (Economic Dispatch)

Consider a two-stage economic dispatch with uncertainty about demand and renewables.

Need to ensure we can meet the second-stage demand but put ourselves in a position to obey ramping constraints while minimizing cost.

1. Dispatch generation in hour 1;
2. Get better forecasts of demand and renewables;
3. Re-dispatch (possibly using expensive peaking generators to compensate if lower-cost generation is not possible).
...
$$\min_x \underbrace{f(x)}_{\text{first-stage objective}} + \mathbb{E}_\xi[\underbrace{Q(x; \xi)}_{\substack{\text{recourse objective} \\ \text{after realization} \\ \text{of } \xi}}]$$
...
## Deterministic Solution With Average Yields
...
## Optimal Solution With High Yields

Variable | Wheat | Corn | Beets
:----- | :-----: | :----: | :-----:
Area (acres) | 183.33 | 66.67 | 250
Yield (T) | 550 | 240 | 6000 
Sales (T) | 350 | -- | 6000 
Purchased (T) | -- | -- | --

This solution yields a profit of \$167,667.

## Optimal Solution With Low Yields

Variable | Wheat | Corn | Beets
:----- | :-----: | :----: | :-----:
Area (acres) | 100 | 25 | 375
Yield (T) | 200 | 60 | 6000 
Sales (T) | -- | -- | 6000 
Purchased (T) | -- | 180 | --

This solution yields a profit of \$59,950.

## Framing This As A Two-Stage Problem

- Make some initial planting decision;
- See what the yields look like;
- Purchase as needed to offset any shortfall.
...
This is a simple (one-layer) tree, but if we had additional uncertainties (e.g. demand) we could compute the joint probabilities of each realization.
:::
::: {.column width=45%}
...
**Wednesday**: Handling Multiple Objectives

**Monday**: Sensitivity Analysis and Robustness

## Assessments

**HW5**: Assigned; Will be due last week of class (12/4).

**Reading**: @Ackoff1979-ix