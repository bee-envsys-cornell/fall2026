...
Probability distributions associate a probability to every event under consideration (the *event space*) and have to follow these rules.
...
- Estimate $$\mathbb{E}_p\left[h(x)\right] = \int_{x \in X} h(x)p(x)dx  \approx \frac{1}{N} \sum_{n=1}^N h(x^n)$$
