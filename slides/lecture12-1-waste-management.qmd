...
$$Y_j = \begin{cases}0 & \text{if facility not operated} \\ 1 & \text{if facility operated} \end{cases}$$

## Objective With Indicator Variables

"Original" (incorrect) objective:

$$\min_{W_{ij}} \sum_i \sum_j a_{ij}\,l_{ij}\,W_{ij} + \sum_j \left[c_j + b_j \sum_i W_{ij}\right]$$

New objectives with indicator variables:

::: {.fragment .fade-in}
$$
\min_{W_{ij}, {Y_j}} \sum_i \sum_j a_{ij}\,l_{ij}\,W_{ij} + \sum_j \left[\mathbf{c_jY_j} + \sum_i b_jW_{ij}\right]$$

:::

## Indicator Variable Constraints

What criteria can we use to determine whether a facility should operate ($Y_j = 1$)?

::: {.fragment .fade-in}

$$Y_j = \begin{cases}0 & \text{if} \sum_{i \in I} W_{ij} = 0 \\[0.5em] 1 & \text{if} \sum_{i \in I} W_{ij} + \sum_{k \neq j} R_{kj} > 0  > 0\end{cases}$$
:::
...
$$
\begin{align}
\min_{W_{ij}, {Y_j}} & \sum_i \sum_j a_{ij}\,l_{ij}\,W_{ij} + \sum_j \left[c_jY_j + \sum_i b_jW_{ij}\right] \\
\text{subject to:} \quad & Y_j = \begin{cases}0 & \text{if } \sum_{i \in I} W_{ij} + \sum_{k \neq j} R_{kj} = 0 \\ 1 & \text{if } \sum_{i \in I} W_{ij} + \sum_{k \neq j} R_{kj} > 0 \end{cases}  \\
...
```julia
@variable(waste, Y[1:3], Bin)
@constraint(waste, commit2, !Y[2] => {W[1,2] + W[2,2] == 0})
```

defines $Y_j$ as a binary indicator variable and implements 
$$Y_2 = \begin{cases}0 &\quad \text{if } W_{21} + W_{22} = 0 \\ 1 & \quad \text{else} \end{cases}
$$
...
- **Wednesday**: Prelim 2
- **Next Week**: Gaussian Plumes and Air Quality